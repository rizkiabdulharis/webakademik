...
<br>
<?php if($row['aktif']=='Y'){?>
<button type="button" onclick=window.location.href='./media.php?page=addkrs'>Tambah KRS</button>
<?php } ?>


<button type="button" onclick="window.open('report/cetak-krs.php?no_bp=<?php echo $row['no_bp']; ?>','_blank','width=900,height=400')">Cetak KRS</button> <br><br>
...
ORDER BY tbl_krs_mhs.id_krs ASC");
  ...
  ?>
    <tr bgcolor='#FFF'>
      ...
      <td align='center'>
        <a href="media.php?page=krs-delete&id=<?php echo $row['id_krs']; ?>">Hapus</a>
      </td>
    </tr>
  <?php
    $no++;
